...
output "vibe_bucket_website_endpoint" {
  ...
  value       = try(ibm_cos_bucket.vibe_bucket.website_endpoint, null)
}

output "push_cos_url" {
  description = "The URL endpoint for the 'push to COS' IBM Cloud Function action."
  value       = var.enable_functions ? try(ibm_function_action.push_to_cos.target_endpoint_url, null) : null
}

output "push_project_url" {
  description = "The URL endpoint for the 'push to Project' IBM Cloud Function action."
  value       = var.enable_functions ? try(ibm_function_action.push_to_project.target_endpoint_url, null) : null
}
