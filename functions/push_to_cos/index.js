...
/**
 * push_to_cos: writes provided HTML to COS as index.html using HMAC (S3 SigV4).
 * Params expected (provided by Terraform default params):
 *  - bucket_name, region, cos_endpoint, access_key_id, secret_access_key
...
exports.main = async (params) => {
  ...
  const html = json.html || '<!doctype html><html><body><h1>Empty vibe</h1></body></html>';

  const service = 's3';
  const host = cos_endpoint;
  const method = 'PUT';
  const path = `/${bucket_name}/index.html`;
  ...
  if (res.statusCode >= 200 && res.statusCode < 300) {
    return {
      ...
      body: JSON.stringify({ status: 'ok', action: 'cos', object: 'index.html' })
    };
  } else {
    return {
      statusCode: 502,
